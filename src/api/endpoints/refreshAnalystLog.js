...
const refreshAnalystLogEndpoint = async (req, res) => {
  console.log('=== /refreshAnalystLog endpoint called ===');
  console.log(`Query params: ${JSON.stringify(req.query)}`);
  
  const requestStartTime = Date.now();
  const { generateRating, test, tickers, priceTarget, frame, quote, setAll } = req.query;
  ...
  try {
    ...
    const logResult = await refreshAnalystLog({
      ...
      frame: frame === 'true',
      quote: quote === 'true',
      setAll: setAll === 'true'  // If true, overwrite all values including existing ones
    });
