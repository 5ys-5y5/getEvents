...
**[IMPLEMENTED]** 시스템 설정 관리 웹 인터페이스

**Features**:
1. **Configuration Management**: appConfig.js 실시간 편집 및 저장
   - Scheduler settings (CRON, timezone, enabled)
   - Batch processing (batch sizes, delays)
   - API rate limits (requests/min, timeouts, retries)
   - Adaptive rate limiting (thresholds, batch sizes, delays)
   - Database limits (query result limits)
   - Return cap settings (maxCap, lowCap for dashboard)

2. **Environment Variable Management**: 암호화된 환경 변수 저장/조회
   - FMP API Key, Supabase credentials
   - 암호화 저장 (envEncryption.js)

3. **Holiday Calendar Update**: Market holidays 수동 업데이트 버튼

**Endpoints**:
- `GET /control` - Control panel UI (requires api_key)
- `POST /control/save` - Save appConfig.js updates
...
**Implementation**: `src/api/endpoints/control.js`

### Symbol Cache Management

**[IMPLEMENTED]** 심볼 캐시 조회 및 관리 엔드포인트

**Endpoints**:
- `GET /symbolCache` - View cached symbols with filters (sector, industry)
- `GET /symbolCache/search?q={query}` - Search symbols by ticker or name
- `GET /symbolCache/stats` - Symbol cache statistics (total, by sector, last updated)
- `GET /symbolCache/check/{ticker}` - Check if ticker exists in cache
- `GET /refreshSymbolCache` - Force refresh symbol cache from FMP API

**Implementation**: `src/api/endpoints/symbolCache.js`

### Pattern Analysis (/pattern)

**[IMPLEMENTED]** 이벤트 패턴 분석 대시보드

**Features**:
1. Event pattern detection and storage
2. Visual analytics for recurring patterns
3. Pattern-based trading signal generation

**Implementation**: `src/api/endpoints/pattern.js`

---

## API Call Optimization Requirements

### ⚡ Critical Performance Rule: Minimize API Calls

**All price data retrieval MUST use range queries (from/to) instead of individual date queries.**

**Applies to**:
- `POST /priceTracker` - **Ticker-grouped batch processing** (priceTrackerService.js:354-599, priceTracker.js:234-304)
- `GET /refreshAnalystLog` - **Ticker-grouped batch processing** (analystCacheManager.js:61-189)
- Any endpoint fetching historical prices

### ⚡ Two-Level Optimization Strategy

**All price-fetching endpoints MUST implement both levels**:

**Example - refreshAnalystLog Optimization**:
```javascript
// Published Date: 2022-05-11
// Horizons: D1, D2, D3, ..., D365
// Earliest target: 2022-05-12 (D1)
// Latest target: 2023-05-11 (D365)

// ❌ OLD: 18 separate API calls (one per horizon)
for (const horizon of [1, 2, 3, ..., 365]) {
  await fetchPriceForDate(ticker, targetDate, targetDate+7);  // 18 API calls!
}

// ✅ NEW: 1 API call for entire range
const url = `https://financialmodelingprep.com/api/v3/historical-price-full/${ticker}?from=2022-05-12&to=2023-05-11&apikey=${key}`;
// Returns ALL daily prices from D1 to D365 in single call
// Build Map → O(1) lookup for each horizon
```

**Performance Impact (refreshAnalystLog)**:
- **Before**: 80,000 records × 18 horizons = 1,440,000 API calls
- **After**: 10 unique tickers = 10 API calls
- **Speedup**: 144,000x reduction in API usage

**Performance Impact (priceTracker)**:
- **Before**: 10,000 trades × 14 horizons = 140,000 API calls
- **After**: 100 unique tickers = 100 API calls
- **Speedup**: 1,400x reduction in API usage

**API Specification** (ApiList.json):
```json
{
  "getQuantitiveValuation": {
    "getLastPrice": {
      "service-FMP": {
        "id": "fmp-historical-price-eod",
        "API": "https://financialmodelingprep.com/api/v3/historical-price-full/{ticker}?from={fromDate}&to={toDate}&apikey={fmpApiKey}"
      }
    }
  }
}
...
for (const date of dates) {
  await getHistoricalOHLC(ticker, date);  // N API calls!
}

// ✅ GOOD: Single range API call
const result = await getHistoricalOHLCRange(ticker, fromDate, toDate);
// Returns all dates in single call → build lookup map → match dates
```

**Fallback for Non-Trading Days** (priceTrackerService.js:259-277):
- If exact date not available in response, search backwards up to 7 days
- Prioritize nearest available trading day
- Mark as `usedFallback: true` in result metadata

**Enforcement**:
- All endpoints fetching prices MUST use `getHistoricalOHLCRange()` (fmpClient.js:413-508)
- All batch endpoints MUST group by ticker before API calls
- No loops with individual `getHistoricalOHLC(ticker, singleDate)` calls
- Performance requirement: API calls = number of unique tickers (NOT number of records)

**Summary Table**:

| Endpoint | Records | Unique Tickers | API Calls | Optimization |
|----------|---------|----------------|-----------|--------------|
| **refreshAnalystLog** | 80,000 | 10 | 10 | ✅ Ticker-grouped |
| **POST /priceTracker** | 10,000 | 100 | 100 | ✅ Ticker-grouped |
| **Any price endpoint** | N | T | T | ✅ MUST be ticker-grouped |
...
- **FR-001**: 시스템은 GET /getEvent?startDate={자연수}&endDate={자연수} 요청을 받아 HTTP 200 응답 시 NDJSON 스트림(eventRecord* + metaRecord 1줄) 또는 JSON 형식을 반환해야 한다 (**UPDATED**: format=ndjson 쿼리 파라미터로 NDJSON/JSON 선택 가능, 기본값은 JSON)
...
- **FR-018**: ~~시스템은 D+N 괴리율 계산 시 (publishedDate+N) 날짜의 EOD 가격을 fmp-historical-price-eod API에서 조회해야 한다. 14개의 D+N horizon 지점(0,1,2,3,4,5,6,7,14,21,30,60,180,365)마다 개별 API 호출을 수행한다~~ **✅ OPTIMIZED**: 시스템은 D+N 괴리율 계산 시 **단일 범위 조회**를 사용해야 한다. `calculatePriceTrend` 함수(analystCacheManager.js:61-189)는 가장 이른 D+N 날짜(earliestDate)부터 가장 늦은 D+N 날짜(latestDate)까지 **단 1번의 API 호출**로 모든 horizon 가격을 가져온다. 예: publishedDate=2022-05-11인 경우, `from=2022-05-12&to=2023-05-11` 범위로 1번 호출하여 D1~D365의 모든 가격 데이터를 수집한 후 Map으로 각 horizon에 매칭한다.
- **FR-019**: 시스템은 D+N 날짜가 휴장일일 경우 이전의 가장 가까운 거래일 가격을 사용해야 한다. **IMPLEMENTATION**: 범위 조회 응답에 해당 날짜가 없으면 최대 7일 이전까지 역방향 검색하여 가장 가까운 거래일 데이터를 사용한다. (priceTrackerService.js:259-277의 `findNearestTradingDayData` 함수 참조)
