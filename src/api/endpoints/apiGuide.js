...
<body>
  ...
  <div class="page-wrapper">
    ...
    <!-- 기능 안내 박스 -->
    <div class="note" style="background: linear-gradient(135deg, rgba(0, 102, 204, 0.1), rgba(46, 125, 50, 0.1)); border-color: #0066cc; padding: 20px; margin: 20px 0;">
      <h3 style="color: #0066cc; margin-bottom: 12px;">📚 API Guide 페이지 기능 안내</h3>
      <p style="line-height: 1.7; margin-bottom: 16px;">
        <strong>API Guide</strong>는 Financial Event API의 모든 엔드포인트, 파라미터, 응답 형식을 상세하게 설명하는 개발자 문서입니다.
      </p>
      
      <div style="padding-left: 16px;">
        <p style="margin-bottom: 12px; font-weight: 600;">🎯 주요 섹션</p>
        <ul style="list-style: disc; padding-left: 20px; line-height: 1.8; margin-bottom: 16px;">
          <li><strong>설치 및 실행</strong>: Node.js 환경 설정, 환경변수 구성, 로컬 실행 방법</li>
          <li><strong>환경변수 설정</strong>: FMP_API_KEY, SUPABASE 연결 정보, 암호화 키 설정</li>
          <li><strong>로컬 실행</strong>: 개발 서버 시작 및 테스트 방법</li>
        </ul>
        
        <p style="margin-bottom: 12px; font-weight: 600;">🔌 API 엔드포인트</p>
        <ul style="list-style: disc; padding-left: 20px; line-height: 1.8; margin-bottom: 16px;">
          <li><strong>GET /getEvent</strong>: 재무 이벤트 정보 조회 (Earnings, Dividends, Splits 등)</li>
          <li><strong>GET /getEventLatest</strong>: 최신 이벤트만 조회 (중복 제거 버전)</li>
          <li><strong>GET /getValuation</strong>: 밸류에이션 지표 조회 (PER, PBR, ROE, FCF 등)</li>
          <li><strong>POST /tracker</strong>: Price Tracker 데이터 등록</li>
          <li><strong>GET /trackedPrice</strong>: 등록된 거래 조회</li>
          <li><strong>POST /refreshAnalystLog</strong>: 애널리스트 로그 갱신</li>
        </ul>
        
        <p style="margin-bottom: 12px; font-weight: 600;">📋 요청 파라미터</p>
        <ul style="list-style: disc; padding-left: 20px; line-height: 1.8; margin-bottom: 16px;">
          <li><strong>symbol</strong> (필수): 종목 코드 (예: AAPL, MSFT)</li>
          <li><strong>api_key</strong> (필수): 인증 키</li>
          <li><strong>from / to</strong>: 날짜 범위 지정 (YYYY-MM-DD)</li>
          <li><strong>limit</strong>: 응답 개수 제한</li>
          <li><strong>useCache</strong>: 캐시 사용 여부 (true/false)</li>
        </ul>
        
        <p style="margin-bottom: 12px; font-weight: 600;">📄 응답 형식</p>
        <ul style="list-style: disc; padding-left: 20px; line-height: 1.8; margin-bottom: 16px;">
          <li><strong>JSON</strong>: 일반 JSON 배열 응답</li>
          <li><strong>NDJSON</strong>: Newline-Delimited JSON (스트리밍 대용량 데이터)</li>
          <li><strong>에러 응답</strong>: { error: "message" } 형식</li>
        </ul>
        
        <p style="margin-bottom: 12px; font-weight: 600;">🔍 데이터 주입 스키마</p>
        <ul style="list-style: disc; padding-left: 20px; line-height: 1.8; margin-bottom: 16px;">
          <li><strong>getEventOutputSchema</strong>: 이벤트 데이터의 JSON 스키마</li>
          <li><strong>getValuationOutputSchema</strong>: 밸류에이션 데이터의 JSON 스키마</li>
          <li><strong>필드 설명</strong>: 각 필드의 의미, 타입, 필수 여부</li>
        </ul>
        
        <p style="margin-bottom: 12px; font-weight: 600;">⚠️ 오류 처리</p>
        <ul style="list-style: disc; padding-left: 20px; line-height: 1.8; margin-bottom: 16px;">
          <li><strong>400 Bad Request</strong>: 잘못된 파라미터 (symbol 누락, 날짜 형식 오류 등)</li>
          <li><strong>401 Unauthorized</strong>: api_key 누락 또는 잘못된 키</li>
          <li><strong>404 Not Found</strong>: 존재하지 않는 종목 또는 데이터 없음</li>
          <li><strong>429 Too Many Requests</strong>: Rate Limit 초과</li>
          <li><strong>500 Internal Server Error</strong>: 서버 내부 오류</li>
        </ul>
        
        <p style="margin-bottom: 12px; font-weight: 600;">💡 사용 예시</p>
        <ul style="list-style: disc; padding-left: 20px; line-height: 1.8; margin-bottom: 16px;">
          <li><strong>기본 조회</strong>: <code>/getEvent?symbol=AAPL&api_key=YOUR_KEY</code></li>
          <li><strong>기간 지정</strong>: <code>/getEvent?symbol=AAPL&from=2024-01-01&to=2024-12-31&api_key=YOUR_KEY</code></li>
          <li><strong>캐시 무시</strong>: <code>/getValuation?symbol=MSFT&useCache=false&api_key=YOUR_KEY</code></li>
          <li><strong>NDJSON 스트리밍</strong>: Accept 헤더에 "application/x-ndjson" 추가</li>
        </ul>
        
        <p style="margin-bottom: 12px; font-weight: 600;">🧪 테스트</p>
        <ul style="list-style: disc; padding-left: 20px; line-height: 1.8;">
          <li><strong>Postman / Thunder Client</strong>: REST API 클라이언트로 테스트</li>
          <li><strong>cURL</strong>: 커맨드라인에서 빠른 테스트</li>
          <li><strong>브라우저</strong>: GET 요청은 브라우저 주소창에서 직접 테스트 가능</li>
        </ul>
        
        <p style="font-size: 0.85rem; color: #666666; margin-top: 16px; padding-top: 12px; border-top: 1px solid #e0e0e0;">
          <strong>📖 문서 활용</strong>: 왼쪽 사이드바의 목차를 통해 원하는 섹션으로 빠르게 이동할 수 있습니다. 
          모든 코드 예시는 복사하여 바로 사용 가능하며, 실제 동작하는 API 키를 입력하면 즉시 테스트할 수 있습니다.
        </p>
      </div>
    </div>
    ...
    <section class="section-block">
      ...
      <ul>
        ...
        <li>Supabase 프로젝트 (URL 및 Anon Key)</li>
        <li>Git</li>
      </ul>
...
cp .env.example .env</code></pre>
      ...
      <h3>환경변수 설정 (.env)</h3>
      <pre><code>FMP_API_KEY=your_fmp_api_key
SUPABASE_URL=https://your-project.supabase.co
SUPABASE_ANON_KEY=your_anon_key</code></pre>

      <h3>로컬 실행</h3>
      <pre><code>npm run dev</code></pre>

      <div class="note">
        <strong>중요:</strong> Supabase 자격증명이 없으면 서버가 시작되지 않습니다.
        모든 데이터는 Supabase PostgreSQL 데이터베이스에 저장됩니다.
      </div>
    </section>

    <section class="section-block">
      <h2 id="database-tables">데이터베이스 테이블</h2>

      <p class="section-intro">
        이 API는 Supabase PostgreSQL 데이터베이스를 사용하여 모든 데이터를 저장하고 관리합니다.
        서버 시작 시 Supabase 연결이 필수입니다.
      </p>
      ...
      <table class="metric-table">
        ...
        <tbody>
          <tr>
            <td><code>symbol_cache</code></td>
            <td>거래 가능한 주식 심볼 목록 (약 30,000개 종목)</td>
            <td>7일마다 자동 갱신</td>
            <td>티커 유효성 검증, 동종업계(Peer) 조회</td>
          </tr>
          <tr>
            <td><code>event_cache</code></td>
            <td>최근 <code>/getEvent</code> 호출 결과 (재무 이벤트 데이터)</td>
            <td>GET /getEvent 호출 시 자동 갱신</td>
            <td><code>/getEventLatest</code>에서 즉시 반환</td>
          </tr>
          <tr>
            <td><code>trades</code></td>
            <td>등록된 거래 추적 데이터</td>
            <td>POST /priceTracker 호출 시 자동 갱신</td>
            <td>거래별 D+1~D+14 가격 이력 및 수익률</td>
          </tr>
          <tr>
            <td><code>model_summaries</code></td>
            <td>모델별 성과 요약 통계</td>
            <td>POST /priceTracker 호출 시 자동 갱신</td>
            <td>최적 보유일수, 추천 Cap 임계값</td>
          </tr>
          <tr>
            <td><code>analyst_records</code></td>
            <td>애널리스트 목표가 및 가격 추이 데이터</td>
            <td>GET /refreshAnalystLog 호출 시 갱신</td>
            <td><code>/generateRating</code>에서 D+N 갭률 계산 및 등급 생성</td>
          </tr>
          <tr>
            <td><code>market_holidays</code></td>
            <td>NASDAQ 거래소 휴장일 정보</td>
            <td>서버 시작 시 자동 갱신 (1일 1회)</td>
            <td>D+N 거래일 계산 시 휴장일 제외</td>
          </tr>
        </tbody>
      </table>
      ...
      <h4>symbol_cache</h4>
      <pre><code>CREATE TABLE symbol_cache (
  ...
  updated_at TIMESTAMPTZ DEFAULT NOW()
      ...
      <h4>event_cache</h4>
      <pre><code>CREATE TABLE event_cache (
  start_date TEXT,
  end_date TEXT,
  from_date TEXT,
  to_date TEXT,
  ...
  returns JSONB,
  meta JSONB,
  PRIMARY KEY (position, model_name, ticker, purchase_date)
);</code></pre>

      <h4>analyst_records</h4>
      <pre><code>CREATE TABLE analyst_records (
  ticker TEXT,
  published_date TEXT,
  analyst_name TEXT,
  ...
  price_trend JSONB,
  -- ... 기타 필드
  PRIMARY KEY (ticker, published_date, analyst_name)
);</code></pre>

      <h4>market_holidays (거래소 휴장일)</h4>
      <pre><code>CREATE TABLE market_holidays (
  exchange TEXT,           -- 거래소 코드 (NASDAQ, NYSE 등)
  date DATE,               -- 휴장일 날짜
  name TEXT,               -- 휴장일 명칭 (예: Thanksgiving Day)
  is_closed BOOLEAN,       -- 완전 휴장 여부
  adj_open_time TEXT,      -- 조정된 개장 시간 (조기 폐장 시)
  adj_close_time TEXT,     -- 조정된 폐장 시간 (조기 폐장 시)
  is_fully_closed BOOLEAN, -- 완전 휴장 여부
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  PRIMARY KEY (exchange, date)
);</code></pre>
      <p style="font-size: 0.9rem; color: var(--text-muted);">
        FMP API <code>holidays-by-exchange</code>에서 자동으로 데이터를 가져와 저장합니다.
        서버 시작 시 및 Control Panel에서 수동으로 업데이트할 수 있습니다.
      </p>

      <div class="note">
        <strong>PostgreSQL 이점:</strong>
        <ul>
          <li>동시성 처리: 트랜잭션 기반으로 데이터 무결성 보장</li>
          <li>확장성: 대용량 데이터 처리에 최적화</li>
          <li>JSONB: 유연한 스키마 + 인덱싱 지원</li>
          <li>Row Level Security: Supabase 보안 정책 적용 가능</li>
      </ul>
      </div>
    </section>
    ...
    <section class="section-block">
      ...
      <div class="endpoint">
        ...
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/getEvent?startDate=3&endDate=7</code><br>
        ...
        <h4>데이터 수집 프로세스</h4>
        <ul>
          <li><strong>심볼 캐시 로드:</strong> 거래 가능한 주식 심볼 목록을 DB <code>symbol_cache</code> 테이블에서 로드 (7일마다 자동 갱신)</li>
          ...
          <li><strong>저장:</strong> 결과를 DB <code>event_cache</code> 테이블에 저장</li>
        </ul>
  ...
  "events": [
    {
      ...
      <div class="endpoint">
        ...
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/getEventLatest</code><br>
        ...
        <h4>데이터 처리 프로세스</h4>
        <ul>
          <li>DB <code>event_cache</code> 테이블에서 최신 데이터 조회</li>
          <li>데이터 유효성 검증</li>
          <li>캐시 데이터 반환</li>
        </ul>
        ...
        <div class="note">
          <ul>
            <li>DB에 데이터가 없으면 404 에러 반환 → 먼저 <code>/getEvent</code> 호출 필요</li>
      ...
      <div class="endpoint">
        ...
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/getValuation?tickers=AAPL,MSFT&cache=false</code><br>
  ...
  "valuations": [
    {
      ...
      <div class="endpoint">
        ...
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/refreshAnalystLog</code><br>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/refreshAnalystLog</code>

        <h4>기능 설명</h4>
        <p>애널리스트 목표가 데이터를 수집하고, 과거 주가 추세(priceTrend)를 채워서 DB <code>analyst_records</code> 테이블을 생성/업데이트합니다.</p>
        ...
        <div class="step-process">
          ...
          <ul>
            ...
            <li>기존 DB analyst_records 데이터와 병합 (과거 데이터 보존)</li>
        ...
        <h4>데이터 저장</h4>
        <ul>
          <li><code>DB: analyst_records</code>: 애널리스트 목표가 + 과거 주가 추세 데이터</li>
          <li>생성된 등급은 API 응답으로 반환 (generateRating=true 시)</li>
        </ul>
        ...
        <h4>Adaptive Rate Limiting (동적 속도 조절)</h4>
        <p>API 호출 속도를 자동으로 최적화하여 Rate Limit(250 req/min)에 도달하지 않으면서 최대 처리량을 달성합니다.</p>
        ...
        <h4>터미널 로그 출력 형식</h4>
        <div class="response-info">
          <strong>priceTarget=true 실행 시:</strong>
          <pre><code>[Price Target] 50/4451 (1%) | Elapsed: 5s | Rate: 45/250 | Mode: Aggressive (usage < 30%)
[Price Target] 100/4451 (2%) | Elapsed: 10s | Rate: 82/250 | Mode: Aggressive (usage < 30%)</code></pre>

          <strong>quote=true 실행 시:</strong>
          <pre><code>[Step 3] 500 records need quote updates (200 skipped - already complete)
[Step 3] Total price_trend slots to fill: 6000
[Quote Fill] Records: 20/500 (4%) | PriceTrend: 240/6000 (4%) | Elapsed: 15s | Rate: 85/250
[Quote Fill] Records: 40/500 (8%) | PriceTrend: 480/6000 (8%) | Elapsed: 28s | Rate: 120/250</code></pre>
        </div>

        <div class="note">
          <strong>로그 해석:</strong>
          <ul>
            <li><code>Records: N/M</code>: 처리된 레코드 수 / 총 레코드 수</li>
            <li><code>PriceTrend: N/M</code>: 채워진 price_trend 슬롯 수 / 총 슬롯 수</li>
            <li><code>Rate: N/250</code>: 현재 1분 내 API 호출 수 / 분당 최대 허용량</li>
            <li><code>Mode</code>: 현재 적용 중인 속도 조절 모드</li>
          </ul>
        </div>
      </div>
      ...
      <div class="endpoint">
        ...
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/generateRating</code><br>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/generateRating</code>

        <h4>기능 설명</h4>
        <p>DB <code>analyst_records</code> 테이블에서 애널리스트별 목표가 정확도와 가격 추이 통계를 계산합니다.
        ...
        <h4>처리 프로세스</h4>
        <ul>
          <li><strong>1단계:</strong> DB <code>analyst_records</code> 테이블에서 모든 애널리스트 리포트 수집</li>
          ...
          <li><strong>5단계:</strong> 계산된 등급 데이터 반환</li>
        </ul>
  ...
  "analysts": {
    ...
    <!-- Symbol Cache API Documentation -->
    <section class="section-block">
      <h2 id="symbol-cache-api">Symbol Cache API - 심볼 캐시 관리</h2>

      <p class="section-intro">
        거래 가능한 주식 심볼 목록을 관리하는 API입니다. FMP API의 <code>/v3/stock/list</code>에서 데이터를 가져와
        <code>symbol_cache</code> 테이블에 저장합니다. 다른 API에서 티커 유효성 검증, 동종업계 조회 등에 활용됩니다.
      </p>

      <div class="endpoint">
        <h3 class="endpoint-title">6. GET /symbolCache - 심볼 캐시 조회</h3>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/symbolCache</code><br>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/symbolCache</code>

        <h4>기능 설명</h4>
        <p>현재 캐시된 심볼 목록을 조회합니다. 필터링, 검색, 페이지네이션을 지원합니다.</p>
        ...
        <table class="metric-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>선택</td>
              <td>-</td>
              <td>거래소 필터 (NASDAQ, NYSE, AMEX 등)</td>
            </tr>
            <tr>
              <td><code>sector</code></td>
              <td>선택</td>
              <td>-</td>
              <td>섹터 필터 (Technology, Healthcare 등)</td>
            </tr>
            <tr>
              <td><code>industry</code></td>
              <td>선택</td>
              <td>-</td>
              <td>산업 필터 (Software, Biotechnology 등)</td>
            </tr>
            <tr>
              <td><code>q</code></td>
              <td>선택</td>
  ...
  "meta": {
    ...
    "limit": 100,
    "hasMore": true,
    "filters": { "exchange": "NASDAQ", "sector": null },
    "duration": "45ms"
  },
  "symbols": [
    {
      ...
      "type": "stock"
        ...
        <h4>사용 예시</h4>
        <pre><code># 기본 조회 (상위 100개)
GET /symbolCache

# NASDAQ 종목만 조회
GET /symbolCache?exchange=NASDAQ&limit=500

# Technology 섹터 검색
GET /symbolCache?sector=Technology

# 티커/회사명 검색
GET /symbolCache?q=apple</code></pre>
      </div>

      <div class="endpoint">
        <h3 class="endpoint-title">7. GET /refreshSymbolCache - 심볼 캐시 강제 갱신</h3>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/refreshSymbolCache</code><br>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/refreshSymbolCache</code>

        <h4>기능 설명</h4>
        <p>FMP API에서 최신 심볼 목록을 가져와 DB를 갱신합니다. 기본적으로 캐시 유효기간(7일) 내에는 갱신을 건너뜁니다.</p>
        ...
        <table class="metric-table">
          ...
          <tbody>
            <tr>
              ...
              <td>선택</td>
              <td>false</td>
              <td><code>true</code>면 캐시 유효기간 무시하고 강제 갱신</td>
            </tr>
          </tbody>
        </table>
        ...
        <h4>응답 구조</h4>
        <div class="response-info">
          <strong>캐시가 유효한 경우 (갱신 생략):</strong>
          <pre><code>{
  "meta": {
    "action": "skipped",
    "reason": "Cache is still valid",
    "cacheAge": "2.5 days",
    "cacheValidFor": "7 days",
    "symbolCount": 4451,
  ...
  "meta": {
    "action": "refreshed",
    "symbolCount": 4523,
    "lastUpdated": "2025-12-01T15:00:00.000Z",
    "duration": "2341ms",
  ...
  "message": "Symbol cache refreshed successfully. 4523 symbols loaded."
}</code></pre>
        </div>

        <h4>사용 시나리오</h4>
        <ul>
          <li><strong>ApiList.json 필터 변경 후:</strong> <code>country</code> 필터 등 변경 후 즉시 갱신</li>
          <li><strong>신규 상장 종목 반영:</strong> 최신 IPO 종목을 포함하려면 강제 갱신</li>
          <li><strong>캐시 문제 해결:</strong> 특정 티커가 없다고 나올 때 강제 갱신</li>
        </ul>

        <div class="note">
          <strong>주의:</strong> 강제 갱신은 FMP API를 호출하므로 Rate Limit에 영향을 줍니다.
          불필요한 강제 갱신은 피하세요.
        </div>
      </div>

      <div class="endpoint">
        <h3 class="endpoint-title">8. GET /symbolCache/search - 심볼 검색</h3>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/symbolCache/search?q=apple</code>

        <h4>기능 설명</h4>
        <p>티커 또는 회사명으로 심볼을 검색합니다. 검색 결과는 우선순위에 따라 정렬됩니다:</p>
        <ol>
          <li><strong>정확 일치:</strong> 티커가 검색어와 정확히 일치</li>
          <li><strong>시작 일치:</strong> 티커가 검색어로 시작</li>
          <li><strong>포함:</strong> 티커 또는 회사명에 검색어 포함</li>
        </ol>
  ...
  "results": [
    ...
    { "symbol": "APLE", "name": "Apple Hospitality REIT", "exchange": "NYSE", ... }
      ...
      <div class="endpoint">
        <h3 class="endpoint-title">9. GET /symbolCache/stats - 심볼 캐시 통계</h3>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/symbolCache/stats</code>

        <h4>기능 설명</h4>
        <p>심볼 캐시의 전체 통계를 조회합니다. 거래소별, 섹터별, 타입별 분포를 확인할 수 있습니다.</p>

        <h4>응답 구조</h4>
        <div class="response-info">
          <pre><code>{
  "meta": { "timestamp": "2025-12-01T15:00:00.000Z" },
  "cacheSettings": {
    "validForDays": 7,
    "maxSymbols": 50000
  },
  "statistics": {
    ...
    "byType": {
      ...
      <div class="endpoint">
        <h3 class="endpoint-title">10. GET /symbolCache/check/:ticker - 티커 존재 확인</h3>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/symbolCache/check/AAPL</code>

        <h4>기능 설명</h4>
        <p>특정 티커가 심볼 캐시에 존재하는지 확인합니다. 티커 유효성 검증에 활용됩니다.</p>

        <h4>응답 구조</h4>
        <div class="response-info">
          <strong>존재하는 경우:</strong>
          <pre><code>{
  "found": true,
  "symbol": {
    "symbol": "AAPL",
    ...
    "type": "stock"
          ...
          <strong>존재하지 않는 경우:</strong>
          <pre><code>{
  "found": false,
  "ticker": "INVALID",
  "message": "Ticker INVALID not found in symbol cache"
}</code></pre>
        </div>

        <h4>사용 시나리오</h4>
        <ul>
          <li>거래 등록 전 티커 유효성 사전 검증</li>
          <li>특정 종목이 캐시에 포함되어 있는지 확인</li>
          <li>캐시 갱신 필요 여부 판단</li>
        </ul>
      </div>

      <h3>심볼 캐시 데이터 흐름</h3>
      <div class="note">
        <strong>자동 갱신 메커니즘:</strong>
        <ul>
          <li><code>/getEvent</code>, <code>/priceTracker</code> 등 호출 시 내부적으로 <code>getSymbolCache()</code> 실행</li>
          <li>캐시 유효기간(기본 7일)이 지났으면 자동으로 FMP API 호출하여 갱신</li>
          <li>수동 갱신이 필요하면 <code>/refreshSymbolCache?force=true</code> 사용</li>
        </ul>

        <strong>캐시 데이터 소스:</strong>
        <ul>
          <li>FMP API: <code>/v3/stock/list</code> (ApiList.json의 <code>filter.target</code> 설정 참조)</li>
          <li>필터: <code>country=US</code>, <code>exchange=NYSE,NASDAQ,AMEX</code> 등 적용</li>
          <li>저장: Supabase <code>symbol_cache</code> 테이블</li>
        </ul>

        <strong>활용처:</strong>
        <ul>
          <li><code>/getEvent</code>: 이벤트 필터링 (캐시에 있는 종목만 반환)</li>
          <li><code>/priceTracker</code>: 티커 유효성 검증</li>
          <li><code>/getValuation</code>: Peer 종목 조회</li>
        </ul>
      </div>
    </section>
    ...
    <section class="section-block">
      ...
      <div class="endpoint">
        ...
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/priceTracker</code><br>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/priceTracker</code>
        ...
        <ul>
          ...
          <li><strong>D+N 가격 이력 수집:</strong> D+1부터 D+14까지 각 <strong>거래일</strong>의 OHLC(시가/고가/저가/종가) 데이터 조회</li>
          <li><strong>Cap-Aware 수익률 계산:</strong> 포지션별로 일중 고가/저가가 임계값 도달 시 시가 기준 수익률 적용</li>
          <li><strong>DB 저장:</strong> 거래 데이터를 DB <code>trades</code> 테이블에 저장 (upsert)</li>
          <li><strong>모델 성과 재계산:</strong> 해당 모델의 전체 거래 기반으로 최적 보유일수/추천 Cap/평균 수익률 갱신</li>
        </ul>

        <div class="note">
          <strong>📅 거래일 기준 D+N 계산 (Trading Day Based)</strong>
          <p>D+N은 <strong>거래일(Trading Day)</strong> 기준으로 계산됩니다. 주말(토/일)과 NASDAQ 휴장일은 자동으로 건너뜁니다.</p>
          
          <h5>예시: 금요일(2025-09-19) 매수 시</h5>
          <table class="metric-table" style="margin-top: 10px;">
            <thead>
              <tr><th>D+N</th><th>거래일</th><th>설명</th></tr>
            </thead>
            <tbody>
              <tr><td>D+1</td><td>2025-09-22 (월)</td><td>토/일 건너뜀</td></tr>
              <tr><td>D+2</td><td>2025-09-23 (화)</td><td></td></tr>
              <tr><td>D+3</td><td>2025-09-24 (수)</td><td></td></tr>
              <tr><td>...</td><td>...</td><td></td></tr>
              <tr><td>D+14</td><td>2025-10-09 (목)</td><td>14번째 거래일</td></tr>
            </tbody>
          </table>
          
          <h5>휴장일 자동 관리</h5>
          <ul>
            <li>서버 시작 시 FMP API (<code>holidays-by-exchange</code>)에서 NASDAQ 휴장일 정보를 자동으로 가져옵니다.</li>
            <li>휴장일 정보는 <code>market_holidays</code> 테이블에 캐시되어 매일 1회 갱신됩니다.</li>
            <li>Control Panel에서 수동으로 휴장일 업데이트를 트리거할 수 있습니다.</li>
          </ul>
          
          <h5>휴장일 목록 (2025년 예시)</h5>
          <ul style="columns: 2; column-gap: 30px;">
            <li>1/1 - New Year's Day</li>
            <li>1/20 - MLK Jr. Day</li>
            <li>2/17 - Presidents' Day</li>
...
long	MODEL-1	GOOGL	2025-11-22</code></pre>
        ...
        <table class="metric-table">
          ...
          <tbody>
            ...
            <tr>
              <td><code>ticker</code></td>
              <td>대문자 영문만 허용 + DB symbol_cache 테이블에 존재해야 함</td>
        ...
        <table class="metric-table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>404</td>
              <td>ticker가 DB symbol_cache 테이블에 없음</td>
            </tr>
            <tr>
  ...
  -d "long	MODEL-1	AAPL	2025-11-20"</code></pre>
      ...
      <div class="endpoint">
        ...
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">https://getevents.onrender.com/trackedPrice</code><br>
        <code class="endpoint-url" onclick="window.open(this.textContent, '_blank')">http://localhost:3000/trackedPrice</code>
        ...
        <h4>데이터 처리 프로세스</h4>
        <ul>
          <li>DB <code>trades</code> 테이블에서 거래 데이터 조회</li>
          <li>DB <code>model_summaries</code> 테이블에서 모델 요약 조회</li>
  ...
  "modelSummaries": [
    {
      ...
      "meta": {
        ...
        <div class="note">
          <ul>
            <li>DB에 거래 데이터가 없으면 빈 배열 반환 → 먼저 <code>/priceTracker</code>로 거래 등록 필요</li>
...
├── docs/                       # 설정 파일
│   └── config/                 # API 설정 디렉토리
└── .env                        # 환경 변수 (FMP_API_KEY, SUPABASE_URL, SUPABASE_ANON_KEY)</code></pre>
...
2. 심볼 캐시 로드 (거래 가능 종목 목록)
   ↓ src/services/dbManager.js → getSymbolCache()
   ↓ DB symbol_cache 테이블 조회 (없거나 만료 시 FMP API 호출)
...
8. DB 저장
   ↓ src/services/dbManager.js → saveEventCache()
   ↓ DB event_cache 테이블에 저장</code></pre>

        <strong>🗂️ 관련 파일:</strong>
        <ul>
          <li><code>src/api/endpoints/getEvent.js</code> - 엔드포인트 핸들러 (메인)</li>
          <li><code>src/services/dbManager.js</code> - DB 관리 (getSymbolCache, saveEventCache)</li>
          ...
          <li><code>DB: symbol_cache</code> - 거래 가능 종목 목록</li>
          <li><code>DB: event_cache</code> - 이벤트 캐시</li>
        </ul>
      ...
      <h4 id="endpoint-getEventLatest">2️⃣ GET /getEventLatest - 캐시 이벤트 조회</h4><br>
        ...
        <strong>📊 데이터 흐름:</strong>
        <pre><code>1. DB 캐시 조회
   ↓ src/services/dbManager.js → loadEventCache()
   ↓ DB event_cache 테이블에서 최신 데이터 조회
...
3. JSON 응답 반환
   ↓ res.json(cachedData)</code></pre>
        ...
        <ul>
          ...
          <li><code>src/services/dbManager.js</code> - DB 조회 (loadEventCache)</li>
          <li><code>DB: event_cache</code> - 이벤트 캐시 테이블</li>
        </ul>

        <strong>⚠️ 주의사항:</strong>
        <ul>
          <li>API 호출 없음 - 순수 DB 조회</li>
      ...
      <h4 id="endpoint-getValuation">3️⃣ GET /getValuation - 밸류에이션 계산</h4><br>
        ...
        <strong>📊 데이터 흐름:</strong>
        <pre><code>1. 티커 목록 결정
   ↓ cache=true → DB event_cache 테이블에서 추출
...
6. 응답 조합
   ↓ { ticker, price, quantitative, peerQuantitative, qualitative }</code></pre>
        ...
        <ul>
          ...
          <li><code>DB: event_cache</code> - 티커 목록 소스 (cache=true 시)</li>
        </ul>
...
📍 Step 3: quote=true (과거 주가 데이터 채우기)
   ...
   ↓ priceTrend의 null 값만 채우기 (기존 값 보존)</code></pre>
        ...
        <ul>
          ...
          <li><code>src/services/dbManager.js</code> - DB 작업</li>
          <li><code>src/services/fmpClient.js</code> - FMP API 호출</li>
          <li><code>src/lib/configLoader.js</code> - API 설정 로드</li>
          <li><code>src/lib/dateUtils.js</code> - 날짜 계산 (D+N)</li>
          <li><code>DB: symbol_cache</code> - 티커 목록</li>
          <li><code>DB: analyst_records</code> - 애널리스트 기록</li>
        </ul>
...
7. 결과 반환
   ↓ 계산된 등급 데이터를 API 응답으로 반환</code></pre>
        ...
        <ul>
          ...
          <li><code>DB: analyst_records</code> - 입력 데이터</li>
        </ul>

        <strong>⚡ 성능 특성:</strong>
        <ul>
          <li><strong>외부 API 호출: 0개</strong> - 순수 DB 조회 + 계산</li>
          <li>처리 시간: 일반적으로 < 500ms (데이터량에 따라 다름)</li>
          <li>Rate Limit 영향: 없음</li>
          <li>DB 기반 처리로 빠른 반복 실행 가능</li>
        </ul>
    ...
    "John Doe|Goldman Sachs": {
      ...
      <table class="metric-table metric-table--resizable" id="api-endpoints-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>애널리스트 등급 생성</td>
            <td>
              - DB analyst_records 테이블 조회<br>
              - 예측 정확도 평가<br>
              - API 호출 없이 DB 기반 처리<br>
              - 결과를 API 응답으로 반환
            </td>
          </tr>
          <tr>
      ...
      <table class="metric-table metric-table--resizable" id="service-layer-table">
        ...
        <tbody>
          ...
          <tr>
            <td><code>dbManager.js</code></td>
            <td>데이터베이스 관리</td>
            <td>
              - <code>getSymbolCache()</code>: 심볼 캐시 조회/갱신<br>
              - <code>loadEventCache()</code>: 이벤트 캐시 로드<br>
              - <code>saveEventCache(data)</code>: 이벤트 캐시 저장<br>
              - <code>loadTrackedPriceCache()</code>: 거래 데이터 조회<br>
              - <code>mergeTradeRecord()</code>: 거래 레코드 저장<br>
              - Supabase PostgreSQL 기반 CRUD
            </td>
          </tr>
          <tr>
      ...
      <table class="metric-table metric-table--resizable" id="cache-files-table">
        ...
        <tbody>
          <tr>
            ...
            <td>
              거래 가능한 티커 목록<br>
              ticker, sector, industry, data(JSONB)
            </td>
            <td>7일</td>
          </tr>
          <tr>
            ...
            <td>
              재무 이벤트 데이터 캐시<br>
              start_date, end_date, events(JSONB), meta(JSONB)
            </td>
            <td>매 요청마다 갱신</td>
          </tr>
          <tr>
            <td><code>trades</code></td>
            <td>/priceTracker 호출 시</td>
            <td>
              거래 추적 데이터<br>
              position, model_name, ticker, price_history(JSONB)
            </td>
            <td>거래 등록 시</td>
          </tr>
          <tr>
            ...
            <td>
              모델 성과 요약<br>
              model_name, optimal_holding_days, suggested_max_cap
            </td>
            <td>거래 등록 시</td>
          </tr>
          <tr>
            ...
            <td>
              ...
              ticker, analyst_name, price_trend(JSONB)
            </td>
            <td>수동 갱신</td>
          </tr>
        </tbody>
    ...
    <section class="section-block">
      ...
      <table class="metric-table metric-table--resizable" id="faq-table">
        ...
        <tbody>
          ...
          <tr>
            <td>데이터가 많아지면 성능 문제가 생기나요?</td>
            <td>
              Supabase PostgreSQL은 대용량 데이터 처리에 최적화되어 있습니다.
              JSONB 필드에 GIN 인덱스를 추가하면 쿼리 성능을 더욱 향상시킬 수 있습니다.
              ~3만 티커 기준으로 문제없이 처리됩니다.
            </td>
          </tr>
          <tr>
